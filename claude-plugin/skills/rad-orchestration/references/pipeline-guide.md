...
The `radorch pipeline signal` subcommand is the pipeline entry point. All pipeline calls use this canonical form, with `${CLAUDE_PLUGIN_ROOT}` resolving to the orchestration install root at runtime:

```
node "${CLAUDE_PLUGIN_ROOT}/skills/rad-orchestration/scripts/radorch.mjs" pipeline signal --event <event> --project-dir <dir> [--config <path>] [--template <name>]
    [--doc-path <path>]
...
When `data.action` is `invoke_source_control_pr`, spawn **rad-orc:source-control** in PR mode:
