...
// harness-installers/claude-plugin/node_modules/tar/dist/esm/index.min.js
...
function isEntryCurrent(entry, version) {
  ...
  return entry.version === version && entry.channel === "claude-plugin" && typeof entry.installed_at === "string" && entry.last_writer_version === version;
}
...
function loadManifest(pluginRoot, version) {
  const p2 = path3.join(pluginRoot, "manifests", `v${version}.json`);
...
// harness-installers/claude-plugin/lib/install/ui-stop.js
...
function cleanupStagingDir(pluginRoot) {
  try {
    fs8.rmSync(path7.join(pluginRoot, "_install-source"), { recursive: true, force: true });
    fs8.rmSync(path7.join(pluginRoot, "templates"), { recursive: true, force: true });
    fs8.rmSync(path7.join(pluginRoot, "orchestration.yml"), { force: true });
    fs8.rmSync(path7.join(pluginRoot, "ui"), { recursive: true, force: true });
  } catch {
  }
}
function cmpSemver(a, b2) {
  const pa = a.split(/[.-]/).map((p2) => /^\d+$/.test(p2) ? Number(p2) : p2);
  const pb = b2.split(/[.-]/).map((p2) => /^\d+$/.test(p2) ? Number(p2) : p2);
  const n = Math.max(pa.length, pb.length);
  for (let i = 0; i < n; i++) {
    const x = pa[i];
    const y2 = pb[i];
    if (x === void 0) return typeof y2 === "string" ? 1 : -1;
    if (y2 === void 0) return typeof x === "string" ? -1 : 1;
    if (typeof x === "number" && typeof y2 === "number") {
      if (x !== y2) return x < y2 ? -1 : 1;
    } else if (typeof x === "number") return 1;
    else if (typeof y2 === "number") return -1;
    else if (x !== y2) return x < y2 ? -1 : 1;
  }
  return 0;
}
...
async function runInstall(opts) {
  ...
  let uiStopped = false;
  try {
    const pkg = JSON.parse(fs8.readFileSync(path7.join(opts.pluginRoot, "package.json"), "utf8"));
...
function isUnderClaudeCache(pluginRoot) {
  const cacheRoot = path8.join(os3.homedir(), ".claude", "plugins", "cache");
...
async function main() {
  ...
  if (!pluginRoot) {
    logErr("CLAUDE_PLUGIN_ROOT unset");
    return 0;
  }
