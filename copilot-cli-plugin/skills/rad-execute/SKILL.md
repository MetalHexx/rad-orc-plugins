...
1. Run `node "${COPILOT_CLI_PLUGIN_ROOT}/skills/rad-orchestration/scripts/radorch.mjs" project context --project-name {PROJECT_NAME}` and parse fields from the envelope's `data` block. `{PROJECT_NAME}` comes from the /rad-execute argument or conversation context.
...
```
node "${COPILOT_CLI_PLUGIN_ROOT}/skills/rad-orchestration/scripts/radorch.mjs" pipeline signal --event start --project-dir {projectDir}
```
