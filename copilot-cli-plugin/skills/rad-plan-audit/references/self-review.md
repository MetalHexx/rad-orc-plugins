...
|-----------------|----------|----------|
| Requirements | Brainstorming doc *(if present)* | §2.3 terminology; **§2.4 decision encapsulation (must not have forward references to Master Plan, phase plan, task handoff; no deferred-work verbs; no conditional decisions);** structural rules from `${COPILOT_CLI_PLUGIN_ROOT}/skills/rad-create-plans/references/requirements/workflow.md` step 7 (block shape, tags, no placeholders). |
