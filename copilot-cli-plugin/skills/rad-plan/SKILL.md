...
```
node "${COPILOT_CLI_PLUGIN_ROOT}/skills/rad-orchestration/scripts/radorch.mjs" pipeline signal \
  --event start \
...
- Dispatch a fresh subagent with the `rad-plan-audit` skill (full-audit
  ...
  `${COPILOT_CLI_PLUGIN_ROOT}/skills/rad-plan-audit/references/full-audit.md`. The subagent
...
- If `verdict: issues_found`:
    1. Dispatch the `planner` agent with the audit report path, the
       ...
       `${COPILOT_CLI_PLUGIN_ROOT}/skills/rad-plan-audit/references/corrections-workflow.md`.
    ...
    2. Re-invoke the explosion subcommand to regenerate `phases/` and
       `tasks/` from the corrected Master Plan:
           ...
           node "${COPILOT_CLI_PLUGIN_ROOT}/skills/rad-orchestration/scripts/radorch.mjs" plan explode \
