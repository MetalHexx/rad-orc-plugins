...
The `radorch pipeline signal` subcommand is the pipeline entry point. All pipeline calls use this canonical form, with `${COPILOT_CLI_PLUGIN_ROOT}` resolving to the orchestration install root at runtime:

```
node "${COPILOT_CLI_PLUGIN_ROOT}/skills/rad-orchestration/scripts/radorch.mjs" pipeline signal --event <event> --project-dir <dir> [--config <path>] [--template <name>]
    [--doc-path <path>]
